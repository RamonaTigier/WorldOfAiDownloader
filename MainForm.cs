...
namespace Metacraft.FlightSimulation.WoaiDownloader
{
    public partial class MainForm : Form
    {
        ...
        private void DownloadNextPackage()
        {
            ...
            Regex fdlrex = new Regex(@"\=(.*?)\&");
            var fdlvar = fdlrex.Match(downloadUri);
            string fdl = fdlvar.Groups[1].ToString();
            fdl = fdl.ToLower();
            fdl = Path.Combine(txtDownloadFolder.Text, fdl);
            if (File.Exists(fdl))
            {
                AddWarningMessage(Environment.NewLine + "File " + fdl + " already exists, please check!" + Environment.NewLine);
        ...
        void mPackageDownloadClient_DownloadDataCompleted(object sender, DownloadDataCompletedEventArgs e)
        {
            ...
            //AddMessage(Path.Combine(txtDownloadFolder.Text, match.Groups[1].Value) );
            if (!match.Success)
            {
        ...
        private void AddWarningMessage(string error)
        {
            AddMessage(error, Color.OrangeRed);
        }
